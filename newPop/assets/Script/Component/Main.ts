...
export default class Main extends cc.Component {
    ...
    observerAllBtn(closeBtn:boolean):void{
                    ...
                    cc.Tools.sendRequest("AdIntervalShow", "GET", {}).then((res) => {
                        if (res.data.is_show && this.barrageMove) {
                            cc.Tools.showTips(this.node, `<b><color=#ffffff>看完视频 领取更多红包券</c></b>`).then(() => {
                                CocosBridge.JSCallNative("showRewardVideoAd","12")
                            });
                        }
                    })
    ...
    */
    ...
    touchSnow() {
        ...
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_snowman_1"})
        cc.Tools.showTips(this.node, `<b><color=#ffffff>看完视频 领取更多红包券</c></b>`).then(() => {
    ...
    touchRed() {
        if (cc.Tools.userInfo.up_level_num_not_get) {
            ...
            CocosBridge.JSCallNative("playDot",""+{"dot":"click_clickredbag_1"})
   ...
   */
    ...
    showSignLayer() {
        // cc.audioEngine.play(this.effectAudio[3], false, 1);
        this.barrageMove = false;
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_clicksign_1"})
        if (!this.signLayer) {
    ...
    */
    showUnFreezeLayer(e) {
        let target = e.target;
        // cc.audioEngine.play(this.effectAudio[3], false, 1);
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_icetable_1"})
        // 解冻红包新需求
    ...
    showSaveCashLayer() {
        // cc.audioEngine.play(this.effectAudio[3], false, 1);
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_Piggybank_1"})
    ...
    */
    showSetLayer() {
        this.barrageMove = false;
        // cc.audioEngine.play(this.effectAudio[3], false, 1);
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_clicksetting_1"})
    ...
    showGetCashLayer() {
        this.barrageMove = false;
        // cc.audioEngine.play(this.effectAudio[3], false, 1);
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_cash_1"})
    ...
    showLotteryleLayer() {
        this.barrageMove = false;
        // cc.audioEngine.play(this.effectAudio[3], false, 1);
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_turntable_1"})
        let btnLayer = this.content.getChildByName("btn_layer");
    ...
    */
    showStealLayer() {
        this.barrageMove = false;
        // cc.audioEngine.play(this.effectAudio[3], false, 1);
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_steal_1"})
        if (!this.stealLayer) {
        ...
        if (this.clickRedArr.length === 3) {
            ...
            if (total < 800) {
                ...
                if (this.clickRedNumber < 6) {
                    cc.Tools.showTips(this.node, `<b><color=#ffffff>看完视频 领取更多红包券</c></b>`).then(() => {
                        CocosBridge.JSCallNative("showRewardVideoAd","7")
                    });
                    return;
                }
            } else {
    ...
    clickFloate(e) {
        ...
        let target = e.target
        CocosBridge.JSCallNative("playDot",""+{"dot":"click_Floatredbag_1"})
        CocosBridge.JSCallNative("showRewardVideoAd","2")
        // this.scheduleOnce(() => {
        target.active = false;
