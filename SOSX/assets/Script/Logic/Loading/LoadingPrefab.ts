...
export default class LoadingPrefab extends cc.Component {
    ...
    @property(cc.Button)
    btn_switch_tips_left: cc.Button = null;

    @property(cc.Button)
    btn_switch_tips_right: cc.Button = null;

    @property(cc.Label)
    lbl_ready_title: cc.Label = null;

    @property(cc.Label)
    lbl_first_loading: cc.Label = null;
    ...
    onLoad () {
        ...
        this.lbl_pvp_title.node.active = false;
        this.node_cutdown_time.active  = false;
        this.cancleBtn.node.active     = false;
        this.node_card_group.active    = false;
        this.btn_switch_tips_left.node.on("click", this.randomGeneratorTips, this);
        this.btn_switch_tips_right.node.on("click", this.randomGeneratorTips, this);
        ...
        Net.listenLoaclMessage(LOCAL_MESSAGE.LocalMsg_HideCancelMachFightBtn, (param)=>{
            this.cancleBtn.node.active = false;
            this.lbl_first_loading.node.active = !this.node_card_group.active && this._bNewPlayerFirstFight;
            cc.log("hidecancelbtn%%%%%%%%%%%%")
    ...
    start () {
        ...
        // this.moveToPromise = new Promise<void>(resolve=>{
        //     let action = cc.moveTo(0.5, new cc.Vec2(0, 0))
        //     this.nodeForeground.runAction(cc.sequence(action.easing(cc.easeOut(3))
        //         ,cc.callFunc(()=>{
        //             resolve();
        //         })
        //     ));
    ...
    public async loadRes(sceneName:string, func?:()=>Promise<void>, loadFin?:Function, bVisibleNodeInfo: boolean = true) {
        ...
        if(spine){
            if(sceneName.localeCompare('MainScene') == 0){
                spine.setAnimation(0, "idle1", true)
            } else {
                if(sceneName.localeCompare("FightPvEScene") == 0){
                    spine.setAnimation(0, "idle1", true)
                } else {
    ...
    public setCurFightType(type: proto.FightType){
        ...
        case proto.FightType.RookiePvP:
            this.node_card_group.active = false;
            this.lbl_first_loading.node.active = this._bNewPlayerFirstFight;   
            break;
        default:
    ...
    private startPvpInvitationProcess(){
        this.lbl_pvp_title.node.active = true;
    ...
    private setPvpInvitationTitle(){
        ...
        this.lbl_pvp_title.string = titleStr;
    }
    ...
    /* 取消战斗邀请 */
    private cancelFightInvocation(){
        //let param = new proto.Msg_AllianceFightCancelReq()
        //Net.Send(proto.Ptl.AllianceFightCancelReq, param)
    }
