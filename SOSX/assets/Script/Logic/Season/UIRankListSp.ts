...
@ccclass
export default class UIRankListSp extends InfiniteCell {
    @property(cc.Label)
    ...
    public initData(rankLv: number) {
        ...
        this.levelIcon.setTexture(data.RankIcon)
        ...
        this.afterBar.node.active = true
        if (rankLv == tab.Data.RankScoreRewardTable.length -1){
            this.afterBar.node.active = false
        }
        ...
        if (data.RewardCount1) {
            ...
            this.award.getComponent(SeasonRewardItem).setRewardScore(data.Score);
        }
