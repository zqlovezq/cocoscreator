...
cc.log("cocos user info " + e.userInfo);
...
i[c - 1].status ? e.completeBtn(a) : c === e.signDay ? e.selectBtn(a) : e.unSelectBtn(a);
}
});
},
...
e.RedPoolLayer.getChildByName("award_lbl").getComponent(cc.Label).string = o.amount;
...
e.adjusBoomLayout();
e.ResumeGameLayer();
cc.zm.LevelInfo.stage <= 5 && cc.Tools.dot("start_" + cc.zm.LevelInfo.stage);
});
},
...
if (o > 0) for (var c = 0; c < o; c++) {
var a = cc.instantiate(this.Prefab.Mouse), n = (this.itemArea.width - 30) / 2 * 2 * (Math.random() - .5), s = (this.itemArea.height - 30) / 2 * 2 * (Math.random() - .5), r = cc.v2(n, s);
...
var l = Number(i[1]);
if (l > 0) for (var d = 0; d < l; d++) {
var h = cc.instantiate(this.Prefab.DrillMouse), u = (this.itemArea.width - 30) / 2 * 2 * (Math.random() - .5), m = (this.itemArea.height - 30) / 2 * 2 * (Math.random() - .5), g = cc.v2(u, m);
h.parent = this.itemArea;
h.score = 700;
h.setPosition(g);
this.moveMouse(h);
}
...
t.active = !1;
i.active = !1;
cc.zm.LevelInfo.stage <= 5 && cc.Tools.dot("end_" + cc.zm.LevelInfo.stage);
if (1 === this.victory) {
i.active = !0;
