    ...
    execute: function () {
      ...
      _export("HandbookItem", HandbookItem = (_dec = ccclass('HandbookItem'), _dec2 = property(Sprite), _dec3 = property([_crd && CommonItem === void 0 ? (_reportPossibleCrUseOfCommonItem({
        ...
        initData(_tab) {
          ...
          var showType = _tab.ShowType;
          this.node_waiting.active = showType === 2;
          this.node_finish.active = showType === 1;
          this.node_content.destroyAllChildren(); // 显示奖励

          for (var i = 0; i < _tab.RewardItemId.length; i++) {
            var itemId = _tab.RewardItemId[i];
            var itemCount = _tab.RewardItemNum[i];
          ...
          for (var k = 1; k <= 3; k++) {
            var lbl = this["lbl_condition_txt" + k];
            lbl.string = "";
          }
          ...
          if (!funcData.isOpen) {
            ...
            if (_tab.BattleLv) {
              ...
              var ids = (_crd && BattleMainDataControl === void 0 ? (_reportPossibleCrUseOfBattleMainDataControl({
                error: Error()
