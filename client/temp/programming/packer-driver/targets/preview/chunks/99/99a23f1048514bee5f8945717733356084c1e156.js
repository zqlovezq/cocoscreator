    ...
    execute: function () {
      ...
      _export("FightGainBuffControl", FightGainBuffControl = class FightGainBuffControl extends (_crd && AbsControl === void 0 ? (_reportPossibleCrUseOfAbsControl({
        ...
        onFight_Start_Complete() {
          //处理进场送rogueid
          var gainList = this.getListByType((_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            error: Error()
          ...
          for (var i = 0; i < gainList.length; i++) {
            ...
            for (var index = 0; index < conf.CorrespondingId.length; index++) {
              (_crd && EventMgr === void 0 ? (_reportPossibleCrUseOfEventMgr({
                error: Error()
        ...
        onSelect_leader(leaderRole) {
          var list = this.getListByType((_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            error: Error()
        ...
        addId(id) {
          ...
          if (conf) {
            var list = this.getListByType(conf.EffectTarget);
            list.push(conf);
          }
        }
        ...
        hasTypeAndCorrespondingId(type, id) {
          ...
          for (var index = 0; index < list.length; index++) {
            ...
            for (var j = 0; j < v.CorrespondingId.length; j++) {
              var vid = v.CorrespondingId[j];
        ...
        inRole(role) {
          ...
          if (list.length > 0) {
            for (var index = 0; index < list.length; index++) {
              ...
              var heroClass = this.ownClassToHeroClass(v.Own);
        ...
        gainAbsRole(absRole, conf) {
          ...
          for (var index = 0; index < conf.CorrespondingId.length; index++) {
            var v = conf.CorrespondingId[index];
            var skillTab = absRole.info.skillPowers.createTypeAnyId((_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
