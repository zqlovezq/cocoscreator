      ...
      _export("PrestigePop", PrestigePop = (_dec = ccclass('PrestigePop'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(ScrollView), _dec5 = property(Node), _dec6 = property(Button), _dec7 = property(Prefab), _dec8 = property(Prefab), _dec9 = property(Prefab), _dec10 = property(Prefab), _dec11 = property(Node), _dec(_class = (_class2 = class PrestigePop extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        initAttrView() {
          if (this.currTable) {
            ...
            var aTypes = this.currTable.QuestLogAttrType;

            for (var _key in aTypes) {
              var type = aTypes[_key];

              if (type != 0) {
                var value = this.currTable.QuestLogAttrValue[_key];
                var item = this.getAttrItem(Number(_key));
        ...
        updateTaskView() {
          ...
          var level = (_crd && PrestigeData === void 0 ? (_reportPossibleCrUseOfPrestigeData({
            error: Error()
          }), PrestigeData) : PrestigeData).ins.level;
          var isCanLevel = true;

          if (this.currTable) {
            for (var key in this.currTable.TaskIds) {
              var info = (_crd && PrestigeData === void 0 ? (_reportPossibleCrUseOfPrestigeData({
                error: Error()
              ...
              var item = this.getTaskItem(Number(key));
              item.initData(info);
        ...
        updateTilteView() {
          ...
          var maxLevel = this.currTable.ShowLv;

          for (var i = 0; i < 2; i++) {
            var item = this.getTitleItem(i);
            item.initData(true, 1, null);
          }

          for (var _i = 0; _i < maxLevel; _i++) {
            var _item = this.getTitleItem(2 + _i);

            var isUnLock = level >= tables[_i].Level;
            var valeu = 0;

            if (level > tables[_i].Level) {
              valeu = 1;
            } else if (level == tables[_i].Level) {
              valeu = 0.5;
            }

            _item.initData(isUnLock, valeu, tables[_i]);
          }

          for (var _i2 = maxLevel; _i2 < maxLevel + 2; _i2++) {
            var _item2 = this.getTitleItem(_i2 + 2);

            _item2.initData(false, 0, null);
          }
        }
        ...
        getTaskItem(index) {
          ...
          if (!this.taskItems[index]) {
            var node = instantiate(this.taskPrefab);
        ...
        getTitleItem(index) {
          ...
          if (!this.titleItems[index]) {
            var node = instantiate(this.titlePrefab);
        ...
        setScrollPos() {
          var pos = this.currSelectItem.node.getPosition();
