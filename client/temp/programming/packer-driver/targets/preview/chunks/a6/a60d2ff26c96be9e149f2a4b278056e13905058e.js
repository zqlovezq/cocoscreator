    ...
    execute: function () {
      ...
      _export("BattlePassView", BattlePassView = (_dec = ccclass('BattlePassView'), _dec2 = property(Node), _dec3 = property(Node), _dec(_class = (_class2 = class BattlePassView extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        setOnshow() {
          ...
          for (var i = 1; i <= 6; i++) {
            var toggleNode = this.node_toggle_content.getChildByName("toggle" + i);
            var data = (_crd && BattlePassDataMgr === void 0 ? (_reportPossibleCrUseOfBattlePassDataMgr({
              error: Error()
        ...
        switchView(e, customValue) {
          var _this = this;

          return _asyncToGenerator(function* () {
            var tabName = Number(customValue);

            if (e && _this.view_type === tabName) {
              return;
            }
            ...
            _this.view_type = tabName;
            var itemTs = null;

            if (_this.activityNode.getChildByName(customValue)) {
              itemTs = _this.activityNode.getChildByName(customValue).getComponent(_crd && BattlePassItem === void 0 ? (_reportPossibleCrUseOfBattlePassItem({
                error: Error()
              }), BattlePassItem) : BattlePassItem);
            }

            if (!itemTs) {
              var view = yield _this.createView(_this.view_type);

              if (view) {
                var _itemTs = view.getComponent(_crd && BattlePassItem === void 0 ? (_reportPossibleCrUseOfBattlePassItem({
                  error: Error()
                }), BattlePassItem) : BattlePassItem);

                _this.currNode = view;

                _itemTs.onShow(_this.view_type);
              }
            } else {
              itemTs.node.active = true;
              _this.currNode = itemTs.node;
              itemTs.onShow(_this.view_type);
            }
          })();
        }
        ...
        createView(viewName) {
          ...
          return _asyncToGenerator(function* () {
            var viewTab = (_crd && BattlePassDataMgr === void 0 ? (_reportPossibleCrUseOfBattlePassDataMgr({
              error: Error()
