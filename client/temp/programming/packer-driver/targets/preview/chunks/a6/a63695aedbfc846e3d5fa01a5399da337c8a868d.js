  ...
  return {
    ...
    execute: function () {
      ...
      _export("FincaFightView", FincaFightView = (_dec = ccclass('FincaFightView'), _dec2 = property(Label), _dec3 = property(Node), _dec4 = property(Prefab), _dec5 = property([Node]), _dec6 = property(Node), _dec7 = property(Label), _dec8 = property(Label), _dec9 = property(Node), _dec10 = (_crd && ButtonLock === void 0 ? (_reportPossibleCrUseOfButtonLock({
        ...
        checkTeamInfo() {
          var heroChange = false;

          for (var i = 0; i < (_crd && FincaFightData === void 0 ? (_reportPossibleCrUseOfFincaFightData({
            error: Error()
        ...
        refreshOpponentsList(list) {
          ...
          for (var i = 0; i < list.length; i++) {
            var roleInfo = list[i];
            var item = instantiate(this.pfb_list_item);
            item.parent = this.node_list_content;
            var itemTs = item.getComponent(_crd && FincaFightItem === void 0 ? (_reportPossibleCrUseOfFincaFightItem({
              error: Error()
        ...
        on_s2c_GetSimpleRankRsp(msg) {
          ...
          var rankStr = msg.ranking > -1 ? msg.ranking + 1 : (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
            error: Error()
        ...
        setTopRank() {
          for (var i = 0; i < this.rank_nodes.length; i++) {
            var data = this.top_player_list[i];
            this.setRankItem(this.rank_nodes[i], data);
          }
        }

        setRankItem(node, data) {
          var nameTxt = node.getChildByName("name_txt").getComponent(Label);
          var infoTxt = node.getChildByName("info_txt").getComponent(Label);
          var nobodyTxt = node.getChildByName("nobody_txt");
          var palyerHerdItem = node.getChildByName("PlayerHeadItem").getComponent(_crd && PlayerHeadItem === void 0 ? (_reportPossibleCrUseOfPlayerHeadItem({
            error: Error()
          ...
          if (data) {
            ...
            var roleId = data.simple.id;

            if (roleId.indexOf("r_") > -1) {
              // 当前是机器人 SimpleRoleInfo
              var robotId = Number(roleId.slice(-1));
              ...
              var lbl_lv = palyerHerdItem.node.getChildByName("lv_node").getChildByName("lv_txt").getComponent(Label);
              var sp_head = palyerHerdItem.node.getChildByName("head_node").getChildByName("head_img").getComponent(Sprite);
        ...
        showRewardInfo() {
          var rewards = (_crd && FincaFightData === void 0 ? (_reportPossibleCrUseOfFincaFightData({
            error: Error()
