    ...
    execute: function () {
      ...
      _export("BlackTips", BlackTips = (_dec = ccclass('BlackTips'), _dec2 = property(Prefab), _dec(_class = (_class2 = class BlackTips extends Component {
        ...
        AddTips(str) {
          ...
          for (var _child of this.node.children) {
            tween(_child).by(0.2, {
              position: new Vec3(0, _child.getComponent(UITransform).height + 30, 0)
            }).start();
          }

          var item = instantiate(this.prefabItem).getComponent(_crd && BlackTipsItem === void 0 ? (_reportPossibleCrUseOfBlackTipsItem({
            error: Error()
