    ...
    execute: function () {
      ...
      _export("FengyunRankingView", FengyunRankingView = (_dec = ccclass('FengyunRankingView'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property([Node]), _dec5 = property(Node), _dec6 = property(Label), _dec7 = property(Node), _dec8 = property(Prefab), _dec9 = property(Label), _dec10 = property(Node), _dec11 = property(ProgressBar), _dec12 = property(Label), _dec13 = property([Node]), _dec14 = property(Node), _dec(_class = (_class2 = class FengyunRankingView extends (_crd && ViewScreen === void 0 ? (_reportPossibleCrUseOfViewScreen({
        ...
        initRank() {
          ...
          var list = this.rankMsg.rankList;
        ...
        setRankInfo(index, msg) {
          ...
          var headItem = node.getChildByName("PlayerHeadItem");
          var nameLab = node.getChildByName("name_txt").getComponent(Label);
          var infoLab = node.getChildByName("info_txt").getComponent(Label);
          var not = node.getChildByName("nobody_txt");

          if (msg) {
            // let herad=new 
            var sinfo = new (_crd && SimpleRoleInfo === void 0 ? (_reportPossibleCrUseOfSimpleRoleInfo({
              error: Error()
        ...
        initReward() {
          var ranks = this.rewardTab.Ranking;
          var myRewards = [];

          for (var key in ranks) {
            if (ranks[key] != "") {
              var strs = ranks[key].split(";");
              ...
              var item = instantiate(this.rankRewardItemPrefab);
              item.parent = this.rankRewardNode;
              var min = Number(strs[0]);
          ...
          for (var _key in myRewards) {
            (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
              error: Error()
            }), ItemPoolMgr) : ItemPoolMgr).ins.createRewadItem(myRewards[_key], this.myRewaedNode);
          }
        }
        ...
        initTaskInfo() {
          ...
          var _loop = function _loop(key) {
            var task = msg.tasks.find(a => a.id == _this.taskIds[key]);
          ...
          for (var key in this.taskIds) {
            _loop(key);
          }

          for (var _key2 in this.taskBgNodes) {
            this.taskBgNodes[_key2].active = false;
          }
          ...
          if (currTask) {
            var taskInfo = new (_crd && TaskInfo === void 0 ? (_reportPossibleCrUseOfTaskInfo({
              error: Error()
            ...
            for (var _key3 in this.currTaskInfo.taskTable.RewardItemIds) {
              var info = new (_crd && ItemInfo === void 0 ? (_reportPossibleCrUseOfItemInfo({
                error: Error()
              }), ItemInfo) : ItemInfo)();
              info.initItemData(this.currTaskInfo.taskTable.RewardItemIds[_key3], this.currTaskInfo.taskTable.RewardItemNum[_key3]);
              (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
                error: Error()
