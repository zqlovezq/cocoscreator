      ...
      _export("DrugView", DrugView = (_dec = ccclass('DrugView'), _dec2 = property(sp.Skeleton), _dec3 = property(Node), _dec4 = property(Prefab), _dec5 = property(Node), _dec6 = property(Node), _dec(_class = (_class2 = class DrugView extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        onShow() {
          ...
          for (var i = 0; i < (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            ...
            var itemTs = item.getComponent(_crd && DrugScrollItem === void 0 ? (_reportPossibleCrUseOfDrugScrollItem({
              error: Error()
            }), DrugScrollItem) : DrugScrollItem);
            itemTs.setData(elixirTab);
            var usedItemCount = (_crd && HeroTeamControl === void 0 ? (_reportPossibleCrUseOfHeroTeamControl({
              error: Error()
          ...
          var attrDatas = (_crd && HeroTeamControl === void 0 ? (_reportPossibleCrUseOfHeroTeamControl({
            error: Error()
          ...
          if (attrDatas.size !== 0) {
            ...
            var children = this.node_attr.children;

            for (var _k = 0; _k < children.length; _k++) {
              var attrItem = children[_k];
              attrItem.active = false;
            }

            var count = 0;

            for (var [key, value] of attrDatas.entries()) {
              console.log("entries: " + key + ": " + value);
              var _attrItem = children[count];
              _attrItem.active = true;
              var attrType = key;
              var iconPath = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                error: Error()
              }), tab) : tab).getData().HeroAttrClientTableByType.getValue(attrType).Icon;

              var icon_img = _attrItem.getChildByName("icon_img").getComponent(Sprite);

              icon_img.setTexture(iconPath);

              var num_txt = _attrItem.getChildByName("num_txt").getComponent(Label);
