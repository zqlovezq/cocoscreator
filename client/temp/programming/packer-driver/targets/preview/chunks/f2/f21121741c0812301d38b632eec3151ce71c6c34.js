      ...
      _export("FightTestAttr", FightTestAttr = (_dec = ccclass('FightTestAttr'), _dec2 = property(_crd && FightRoleTeam === void 0 ? (_reportPossibleCrUseOfFightRoleTeam({
        ...
        onShow() {
          ...
          var _loop = function _loop() {
            var element = _this.fightTeam.items[index];
            var nn = element.node;
            nn.on(Node.EventType.TOUCH_START, () => {
              _this.showInfo(element.info);
            });
          ...
          for (var index = 0; index < this.fightTeam.items.length; index++) {
            _loop();
          }
        ...
        showInfo(info) {
          ...
          var attrList = [["当前血量", role.info.attrData.hp], ["最大血量", role.info.attrData.maxHp], ["当前护盾", role.info.attrData.shield]];

          for (var iterator of role.info.attrData.attr) {
          ...
          for (var index = 0; index < attrList.length; index++) {
            var _iterator = attrList[index];
            var nn = instantiate(this.attrLab);
            nn.active = true;
            nn.position = v3(150 - (7 - _iterator[1].toString().length) * 15, 0, 0);
            this.attrNode.addChild(nn);
            console.log(_iterator[0], js.formatStr("%s：%s", _iterator[0], _iterator[1]));
            nn.getComponent(Label).string = js.formatStr("%s %s：%s ", _iterator[0], (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).AttrType[_iterator[0]] || 0, _iterator[1]);
          }

          this.triggerNode.destroyAllChildren();
          this.skillGroupNode.destroyAllChildren();
          var skillGroups = [role.info.normalGroup, role.info.weaponeGroup];

          for (var _index = 0; _index < skillGroups.length; _index++) {
            var v = skillGroups[_index];

            if (v == null) {
              continue;
            }

            var _nn = instantiate(this.skillGroupItem);

            _nn.active = true;
            _nn.position = v3();
            this.skillGroupNode.addChild(_nn);
            _nn.getChildByName("Label").getComponent(Label).string = "技能组ID:" + v.Id;
            var nn1 = instantiate(this.skillLayoutItem);
            nn1.active = true;
            nn1.position = v3();

            _nn.getChildByName("skillLayoutItem").addChild(nn1);
        ...
        createSkill(layout, skills) {
          ...
          for (var index = 0; index < skills.length; index++) {
            var v = skills[index];
            var nn = instantiate(this.skillItem);
            nn.active = true;
            nn.position = v3();
            layout.addChild(nn);
        ...
        addtTrigger(skill) {
          for (var index = 0; index < skill.triggerTabs.length; index++) {
            var v = skill.triggerTabs[index];
            var nn = instantiate(this.skillItem);
            nn.active = true;
            nn.position = v3();
        ...
        globleAttr(layout, attrs) {
          ...
          for (var index = 0; index < attrs.length; index++) {
            ...
            layout.addChild(nn);
            var key = attrs[index];
            var value = this.getRoleAttr(key);
