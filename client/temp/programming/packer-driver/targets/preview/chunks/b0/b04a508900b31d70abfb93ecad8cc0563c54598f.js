  ...
  return {
    ...
    execute: function () {
      ...
      _export("RevoltCheatControl", RevoltCheatControl = (_dec = ccclass('RevoltCheatControl'), _dec(_class = (_class2 = class RevoltCheatControl extends (_crd && AbsControl === void 0 ? (_reportPossibleCrUseOfAbsControl({
        ...
        getBoss(bossId) {
          for (var index = this.bossDatas.length - 1; index >= 0; index--) {
        ...
        onBoss_Enter(absRole) {
          var dd = new (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
            error: Error()
        ...
        getHeroList() {
          ...
          for (var index = 0; index < roles.length; index++) {
            ...
            for (var _index = 0; _index < role.info.buffs.length; _index++) {
              var buff = role.info.buffs[_index];
              var buffData = new (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
                error: Error()
        ...
        onFight_Monster_Dead(absRole) {
          if (absRole.info && absRole.info.isBoss) {
            var boss = this.getBossByObjId(absRole.objId);
        ...
        onFight_Initiative_Revive(absRole) {
          for (var index = 0; index < this.bossDatas.length; index++) {
            var boss = this.bossDatas[index];
        ...
        addBulletDamage(bullet, attack, defanse, damageData) {
          var boss = this.getBossByObjId(defanse.objId);
        ...
        createAddBuff(buff) {
          var addBuff = new (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
            error: Error()
          }), proto) : proto).CombatData();
        ...
        addCombatDataByBuff(isAdd, ownerRole, buff) {
          if (ownerRole && ownerRole.isRole()) {
            ...
            var _boss = this.getBossByObjId(buff.owner.objId);

            if (_boss) {
              _boss.combatData.push(isAdd ? this.createAddBuff(buff) : this.createRemoveBuff(buff));
            }
          }
        }

        createRemoveBuff(buff) {
          var removeBuff = new (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
            error: Error()
          }), proto) : proto).CombatData();
