    ...
    execute: function () {
      ...
      _export("AssociationRankPop", AssociationRankPop = (_dec = ccclass('AssociationRankPop'), _dec2 = property(_crd && InfiniteList === void 0 ? (_reportPossibleCrUseOfInfiniteList({
        ...
        switchView(e, type) {
          ...
          this.rank_list = [];
          this.top_list = [];
          var rankList = Number(type) === 1 ? this.bossRoleRank : this.bossGuildRank;
        ...
        createTopItem() {
          for (var i = 0; i < 3; i++) {
            ...
            var itemTs = item.getComponent(_crd && AssociationRankTopItem === void 0 ? (_reportPossibleCrUseOfAssociationRankTopItem({
              error: Error()
        ...
        initStaticView() {
          var canInit = this.rank_list.length > 0;
        ...
        createMyRank() {
          var roleData = this.getMyData();

          if (roleData) {
            this.setMyRank(roleData.data, roleData.ranking);
          } else {
            var rankData = this.createDataByRankId();
        ...
        getMyData() {
          var allRank = this.top_list.concat(this.rank_list);

          if (this.view_type === 1) {
            for (var i = 0; i < allRank.length; i++) {
              var roleData = allRank[i];
            ...
            for (var _i = 0; _i < allRank.length; _i++) {
              var _roleData = allRank[_i];

              if (_roleData.guild.simple.id === (_crd && AssociationData === void 0 ? (_reportPossibleCrUseOfAssociationData({
                error: Error()
