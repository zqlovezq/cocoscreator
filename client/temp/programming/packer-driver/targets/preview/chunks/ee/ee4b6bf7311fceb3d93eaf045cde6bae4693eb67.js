    ...
    execute: function () {
      ...
      _export("AssociationData", AssociationData = (_dec = ccclass('AssociationDataMgr'), _dec(_class = (_class2 = class AssociationData extends (_crd && AbsControl === void 0 ? (_reportPossibleCrUseOfAbsControl({
        ...
        initSkill() {
          ...
          for (var k = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            error: Error()
          ...
          if (this.AssocitionData.skillLevelMap) {
            var maps = this.AssocitionData.skillLevelMap;
        ...
        initRank(msg) {
          ...
          for (var j = 0; j < msg.guildRank.length; j++) {
            var guildRankData = msg.guildRank[j];
          ...
          for (var i = 0; i < this.bossRoleRank.length; i++) {
          ...
          for (var k = 0; k < this.bossGuildRank.length; k++) {
        ...
        refreshSelfRoleRankScore(score) {
          var slefRoleRank = this.bossRoleMap.get((_crd && RoleData === void 0 ? (_reportPossibleCrUseOfRoleData({
            error: Error()
        ...
        getSelfRoleRankInfo() {
          var selfRoleRank = this.bossRoleMap.get((_crd && RoleData === void 0 ? (_reportPossibleCrUseOfRoleData({
            error: Error()
          }), RoleData) : RoleData).ins.id);
          return selfRoleRank;
        } // 获取个人的排行榜排名
        ...
        getSelfGuildRankInfo() {
          var selfGuildRank = this.bossGuildMap.get(AssociationData.ins.getAssocitionSimpleInfo().id);
        ...
        setMemberMap() {
          ...
          if (this.AssocitionData.members.length > 0) {
            for (var i = 0; i < this.AssocitionData.members.length; i++) {
              var member = this.AssocitionData.members[i];
        ...
        refreshMemberInfo(roleId, job, isDelete) {
          ...
          var memberData = this.memberMap.get(roleId);
          ...
          if (job) {
            ...
            if (job === (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              ...
              var leaderData = this.memberMap.get(this.simple.leaderId);
        ...
        getBargainGift(Button) {
          for (var i = 0; i < this.AssocitionData.gifts.length; i++) {
            var gift = this.AssocitionData.gifts[i];
        ...
        setGuildAttr() {
          ...
          this.guildSkillClass.forEach((value, key) => {
            var tabs = this.guildSkillClassTab.get(key);
            ...
            for (var i = 0; i < value; i++) {
              ...
              var _map = this.guildAttrMap.get(key);

              for (var k = 0; k < _tab.AttrTypes.length; k++) {
                var type = _tab.AttrTypes[k];
                var val = _tab.AttrValue[k];
        ...
        getCurSkillTabByClass(heroClass) {
          var tabs = this.guildSkillClassTab.get(heroClass);
          var maxLv = tabs[tabs.length - 1].Level;
          var lv = this.getSkillLvByClass(heroClass) + 1 > maxLv ? maxLv : this.getSkillLvByClass(heroClass) + 1;
        ...
        isMaterialEnough(heroClass) {
          var tabData = this.getCurSkillTabByClass(heroClass);

          for (var i = 0; i < tabData.CostItemIds.length; i++) {
            var costId = tabData.CostItemIds[i];
            var costCount = tabData.CostItemCount[i];
        ...
        isSkillLevelMax(heroClass) {
          var tabs = this.guildSkillClassTab.get(heroClass);
          var maxLv = tabs[tabs.length - 1].Level;
          var lv = this.getSkillLvByClass(heroClass) + 1;
        ...
        addGuildRequests(guildId) {
          var request = new (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
            error: Error()
        ...
        getMemberOffLineTime(memberData) {
          ...
          var loginTime = memberData.lastLoginTime;
          var logoutTime = memberData.lastLogoutTime;
        ...
        getResetSkillMaterial(heroClass) {
          ...
          var lv = this.getSkillLvByClass(heroClass);
          var tabs = this.getSkillTabsByClass(heroClass);

          for (var i = 0; i < tabs.length; i++) {
            var _tab = tabs[i];

            if (_tab.Level <= lv) {
              for (var k = 0; k < _tab.CostItemIds.length; k++) {
                var id = _tab.CostItemIds[k];
                var count = _tab.CostItemCount[k];
        ...
        getIsCanBargain(Button) {
          var giftData = this.getBargainGift(Button);

          if (giftData && giftData.bargainRecords) {
            if (giftData.bargainRecords.length === 0) {
              return true;
            } else {
              for (var i = 0; i < giftData.bargainRecords.length; i++) {
                var record = giftData.bargainRecords[i];
        ...
        getIsMinPrice(Button) {
          ...
          if (giftData && giftData.bargainRecords && giftData.bargainRecords.length > 0) {
            var giftTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).getData().GuildGiftTableById.getValue(giftData.tabId);
            var totalPrice = giftTab.DiamondPrice;
            var minPrice = giftTab.MinPrice;
        ...
        checkFunctionIsOpen(openFunc) {
          ...
          var guildOfTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            error: Error()
          }), tab) : tab).getData().GuildOpenFunctionTableByName.getValue(openFunc);
          var guildLevel = this.simple.level;
          return guildLevel >= guildOfTab.Level;
        }

        showFunctionTips(openFunc) {
          var tips = this.getFunctionTips(openFunc);
        ...
        checkRolePromission(permission) {
          var memberData = this.memberMap.get((_crd && RoleData === void 0 ? (_reportPossibleCrUseOfRoleData({
            error: Error()
          }), RoleData) : RoleData).ins.id);
          ...
          var positionTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            error: Error()
