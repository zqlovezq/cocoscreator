  ...
  return {
    ...
    execute: function () {
      ...
      _export("UIScrollSelect", UIScrollSelect = (_dec = ccclass('UIScrollSelect'), _dec2 = property(Prefab), _dec3 = property(Node), _dec4 = property({
        ...
        onLoad() {
          for (var i = 0; i < 10; i++) {
            var item = instantiate(this.pfb_level_item);
          ...
          for (var _i = 0; _i < this.content.children.length; _i++) {
            this.childs[_i] = this.content.children[_i];
            this.childs[_i].position = v3(this.deltaX * (_i - 1), this.childs[_i].position.y, 0);
          }
        ...
        _setPageBtnsStatus() {
          var isRightEdge = this.currentIndex >= this.childs.length - 1;
          ...
          var isLeftEdge = this.currentIndex <= 0;
        ...
        _checkChildX(child, x) {
          ...
          var scale = (1 - dx / this.deltaX) * (this.centerScale - this.minScale) + this.minScale;
        ...
        _onTouch(event) {
          ...
          if (event.type == Node.EventType.TOUCH_START) {
            ...
            EventHandler.emitEvents(this.selectEvents, _evt);
            return;
          }

          this.hasTouchMove = true;
          var dx = event.getUILocation().x - this.dx;
        ...
        _isMoveEdge() {
          var leftEdge = this.childs[0].position.x >= 0;
          var rightEdge = this.childs[this.childs.length - 1].position.x <= 0;
          return {
            left: leftEdge,
            right: rightEdge
          };
        ...
        _onTouchEnd(event) {
          ...
          var lo = tf.convertToNodeSpaceAR(event.getUILocation());
        ...
        update(dt) {
          ...
          var lx = this.childs[this.moveAim].position.x;
          ...
          var cx = this.childs[this.moveAim].position.x;

          if (lx * cx < 0 && Math.abs(cx) < this.deltaX) {
            this.isTestX = false;

            for (var _i2 = 0; _i2 < this.childs.length; _i2++) {
              if (Math.abs(this.childs[_i2].position.x) <= Math.abs(stepx)) {
                this.currentIndex = _i2;
                break;
              }
            }

            for (var _i3 = 0; _i3 < this.childs.length; _i3++) {
              this._checkChildX(this.childs[_i3], this.childs[_i3].position.x + tox - x);
            }
