  ...
  return {
    ...
    execute: function () {
      ...
      _export("LangMgr", LangMgr = class LangMgr extends (_crd && AbsMgr === void 0 ? (_reportPossibleCrUseOfAbsMgr({
        ...
        InitData() {
          this.worldTable = {};
          var jsonAsset = resources.get(this.nowPath, JsonAsset);
          var len = jsonAsset.json.WordTable.length;
          var v;
        ...
        static transformPath(path) {
          if (this.checkPath(path)) {
            var pattern = /world_lang_(.*?)\//;
            var replacement = LangMgr.ins.nowPath + '/';
            var result = path.replace(pattern, replacement);
            return result;
          }
