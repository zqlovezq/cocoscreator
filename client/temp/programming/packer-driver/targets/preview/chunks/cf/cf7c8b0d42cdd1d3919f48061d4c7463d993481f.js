          ...
          var _loop = function _loop() {
            ...
            if (t >= 1) {
              ...
              var _itemNode = (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
                error: Error()
              }), ItemPoolMgr) : ItemPoolMgr).ins.createItem(item, _this.exchangeNode);

              _this.itemNodes.push(_itemNode);

              _itemNode.getComponent(_crd && CommonItem === void 0 ? (_reportPossibleCrUseOfCommonItem({
                error: Error()
              }), CommonItem) : CommonItem).setShowNum(t * bookfraTab.BaseAmount);
            }
          };

          for (var key in this.canExchangItems) {
            _loop();
          }

          for (var _key in rewards) {
            var itemNode = (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
              error: Error()
            }), ItemPoolMgr) : ItemPoolMgr).ins.createItem(rewards[_key], this.rewardNode);
