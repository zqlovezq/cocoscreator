    ...
    execute: function () {
      ...
      _export("RedMgr", RedMgr = class RedMgr {
        ...
        constructor() {
          ...
          for (var key in _crd && RedDotType === void 0 ? (_reportPossibleCrUseOfRedDotType({
            error: Error()
        ...
        static refreshEvent(event, state) {
          ...
          var targetFb = RedMgr.ins.calculateFbs[event];
        ...
        judge(event) {
          for (var index = 0; index < this.observersNodes.length; index++) {
            var v = this.observersNodes[index];
            ...
            if (v.eventNames[event]) {
              var toAdd = this.isRed(event, v.eventNames[event].child, v.eventNames[event].child2);
              v.eventNames[event].isActive = toAdd;
              var isActive = false;

              for (var key in v.eventNames) {
                var v1 = v.eventNames[key];
        ...
        isRed(key, child, child2) {
          ...
          var eventState = this.states[key];
          var toAdd = false;
          ...
          if (typeof eventState == "boolean") {
            ...
            if (child == "all") {
              for (var _key in eventState) {
                if (eventState[_key]) {
                  if (typeof eventState[_key] == "object") {
                    for (var stateKey in eventState[_key]) {
                      if (eventState[_key][stateKey]) {
                        toAdd = true;
                        break;
                      }
                    }
              ...
              if (eventState[child]) {
                if (typeof eventState[child] == "object") {
                  if (child2 == "all") {
                    for (var _key2 in eventState[child]) {
                      if (eventState[child][_key2]) {
                        toAdd = true;
                        break;
                      }
                    }
        ...
        static remove(node) {
          ...
          for (var index = self.observersNodes.length - 1; index >= 0; index--) {
            var v = self.observersNodes[index];
        ...
        getRedNode(node, transform) {
          ...
          if (tmpNode == null) {
            ...
            var uitrans = tmpNode.addComponent(UITransform);
            ...
            var spr = tmpNode.addComponent(Sprite);
