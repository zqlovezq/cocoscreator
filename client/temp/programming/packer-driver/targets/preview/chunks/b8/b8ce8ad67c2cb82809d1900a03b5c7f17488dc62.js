    ...
    execute: function () {
      ...
      _export("HeroRoadView", HeroRoadView = (_dec = ccclass('HeroRoadView'), _dec2 = property(Label), _dec3 = property(Node), _dec4 = property(Prefab), _dec(_class = (_class2 = class HeroRoadView extends (_crd && ComponentBase === void 0 ? (_reportPossibleCrUseOfComponentBase({
        ...
        initView() {
          ...
          var collectionDatas = msg.collectionData;

          var getProgress = type => {
            for (var key in collectionDatas) {
          ...
          for (var key in tables) {
            ...
            var com = node.getComponent(_crd && HeroRoadItem === void 0 ? (_reportPossibleCrUseOfHeroRoadItem({
              error: Error()
            }), HeroRoadItem) : HeroRoadItem);
            this.items.push(com);
            var state = 0;
            var index = msg.rewardList.indexOf(tables[key].Id);
        ...
        updateView() {
          ...
          for (var key in this.items) {
            var table = this.items[key].table;
            var state = 0;
            var index = msg.rewardList.indexOf(table.Id);
        ...
        updateTimer(closeTime) {
          var lastTimer = Number(closeTime) - Math.floor((_crd && RoleData === void 0 ? (_reportPossibleCrUseOfRoleData({
            error: Error()
          }), RoleData) : RoleData).ins.getServerUtcTime());

          if (lastTimer > 0) {
            var ret = (_crd && TimeUtil === void 0 ? (_reportPossibleCrUseOfTimeUtil({
              error: Error()
            }), TimeUtil) : TimeUtil).formaterWithOutSecond3(lastTimer);
            var day = ret.day ? ret.day : 0;
            var hours = ret.hours ? ret.hours : 0;
