    ...
    execute: function () {
      ...
      _export("SkillTriggerControl", SkillTriggerControl = (_dec = ccclass('SkillTriggerControl'), _dec(_class = (_class2 = class SkillTriggerControl extends (_crd && AbsControl === void 0 ? (_reportPossibleCrUseOfAbsControl({
        ...
        onSkillTrigger(absInfo, type, parms) {
          ...
          var tempSkills = tempTypeSkills[type];
          tempSkills.length = 0;
          var skillGroup = absInfo.normalGroup;

          if (skillGroup == null) {
            return;
          }

          var isInNormal = false;
          var skill;
          ...
          if (absInfo.weaponeGroup) {
            //招式
            for (var _j = 0; _j < absInfo.weaponeGroup.skills.length; _j++) {
              skill = absInfo.weaponeGroup.skills[_j];
          ...
          for (var index = 0; index < absInfo.takeSkills.length; index++) {
          ...
          var behaviorInfo, conditionInfo;

          for (var ii = 0; ii < tempSkills.length; ii++) {
            var v = tempSkills[ii];
            ...
            for (var _j2 = 0; _j2 < v.triggerTabs.length; _j2++) {
              skillTriger = v.triggerTabs[_j2];
              ...
              var isSuccess = skillTriger.isTriggerChanceSucceed(absInfo.attrData.getPrdCount(skillTriger.Id));
              ...
              if (!isSuccess) {
                ...
                if (skillTriger.Behavior.length) {
                  ...
                  var isParmSucceed = this.isTriggerCondition(conditionInfo, skillTriger, v, parms);
                  ...
                  if (isParmSucceed) {
                    ...
                    for (var k = 0; k < skillTriger.Behavior.length; k++) {
        ...
        isParametersSucceedByAttackCount(absInfo, skillTrigger) {
          var bool = skillTrigger.isParametersSucceed(absInfo.normalGroup.triggerAttackCount);
        ...
        isParametersSucceedByInfoAttr(absInfo, skillTrigger, attrType, isClear) {
          if (isClear === void 0) {
            isClear = true;
          }

          SkillTriggerControl.print("isParametersSucceedByInfoAttr", attrType, absInfo.getObjAttr(attrType), skillTrigger.Parameters);
          var bool = skillTrigger.isParametersSucceed(absInfo.getObjAttr(attrType));
