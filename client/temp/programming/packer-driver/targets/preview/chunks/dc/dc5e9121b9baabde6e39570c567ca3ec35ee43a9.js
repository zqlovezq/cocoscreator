    ...
    execute: function () {
      ...
      _export("WeaponInfoItem", WeaponInfoItem = (_dec = ccclass('WeaponInfoItem'), _dec2 = property(Sprite), _dec3 = property(Sprite), _dec4 = property(Label), _dec5 = property(_crd && HeroStar === void 0 ? (_reportPossibleCrUseOfHeroStar({
        ...
        initView() {
          ...
          if (this.rogueInfo) {
            var BookDictionary = (_crd && RareBookData === void 0 ? (_reportPossibleCrUseOfRareBookData({
              error: Error()
            ...
            if (this.info.bookTable.Id === BookDictionary.PhaseOneBook) {
              ...
              var heroIds = this.caleRecommendHeros(this.rogueInfo);
              this.createHeroIcon(heroIds);
            }
          } else {
            // 通过bookid
            var Heros = [];
            var buildArr = this.getExtraIds(this.info.bookTable);
        ...
        createHeroIcon(heroIds) {
          ...
          if (heroIds.length > 0) {
            for (var i = 0; i < heroIds.length; i++) {
              var icon = instantiate(this.sp_recommend_icon.node);
              this.node_heros_content.addChild(icon);
              icon.active = true;
              var iconPath = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                error: Error()
        ...
        caleRecommendHeros(rogueInfo) {
          ...
          if (!rogueInfo.heroItemId) {
            ...
            var heroItemIds = this.getHeroIdsByRougeInfos(rogueInfos);
            return heroItemIds;
          } else {
            return [];
          }
        ...
        getHeroIdsByRougeInfos(rogueInfos) {
          ...
          for (var k = 0; k < rogueInfos.length; k++) {
            ...
            if (bookTab.Builds !== (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              ...
              var buildArr = this.getTeamIdBuilds(bookTab.Builds);
          ...
          for (var i = 0; i < rogueInfos.length; i++) {
            var rogueInfo = rogueInfos[i];

            var _bookTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).getData().BookTableById.getValue(rogueInfo.rogueTab.BookId);

            if (_bookTab.Builds === (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).Builds.Builds_Core || _bookTab.Builds === (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).Builds.Builds_Skill) {
              var heroInfo = (_crd && HeroTeamControl === void 0 ? (_reportPossibleCrUseOfHeroTeamControl({
                error: Error()
              }), HeroTeamControl) : HeroTeamControl).ins.getHeroByClass(Number(_bookTab.Class));
        ...
        getExtraIds(bookTab) {
          ...
          var _buildArr = this.getTeamIdBuilds(bookTab.Builds);
          ...
          for (var i = 0; i < _buildArr.length; i++) {
