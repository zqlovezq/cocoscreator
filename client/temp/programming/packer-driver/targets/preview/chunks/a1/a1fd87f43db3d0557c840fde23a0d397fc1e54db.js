    ...
    execute: function () {
      ...
      _export("HeroSkillItem", HeroSkillItem = (_dec = ccclass('HeroSkillItem'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property(Sprite), _dec5 = property(Node), _dec6 = property([Node]), _dec(_class = (_class2 = class HeroSkillItem extends Component {
        ...
        initData(type, heroInfo) {
          ...
          var _skillData = heroInfo.getHeroSkillMap().get(type);

          var _skillIcon = heroTab["SkillIcon" + type]; // let _bgIcon = heroInfo.heroAptitudeTable.SkillBg;

          for (var i = 0; i < this.nodes_skill.length; i++) {
            var node = this.nodes_skill[i];
            node.active = false;
          ...
          if (type === (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            ...
            for (var _i = 0; _i < _skillData.length; _i++) {
              var HeroStarUpTab = _skillData[_i];
        ...
        setSkillImgActive(isActive) {
          ...
          for (var i = 0; i < this.nodes_skill.length; i++) {
            var node = this.nodes_skill[i]; // let child1 = node.children[1];

            var child2 = node.children[2]; // child1.active = isActive;
