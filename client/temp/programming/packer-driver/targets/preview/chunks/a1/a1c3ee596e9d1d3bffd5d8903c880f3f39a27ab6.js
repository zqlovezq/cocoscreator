  ...
  return {
    ...
    execute: function () {
      ...
      _export("FirstRechargePop", FirstRechargePop = (_dec = ccclass('FirstRechargePop'), _dec2 = property(RichText), _dec3 = property(Sprite), _dec4 = property(Node), _dec5 = property(Node), _dec6 = property(Node), _dec7 = property(Node), _dec8 = property(Label), _dec9 = property(sp.Skeleton), _dec10 = property(sp.Skeleton), _dec11 = property(Sprite), _dec12 = property(Node), _dec13 = property([Node]), _dec14 = property(Node), _dec15 = property(Label), _dec(_class = (_class2 = class FirstRechargePop extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        setToggles() {
          var tables = (_crd && PayData === void 0 ? (_reportPossibleCrUseOfPayData({
            error: Error()
          }), PayData) : PayData).ins.getFirstRechargeTabs();
          var boughtGoodsIds = (_crd && PayData === void 0 ? (_reportPossibleCrUseOfPayData({
            error: Error()
        ...
        initView() {
          ...
          return _asyncToGenerator(function* () {
            ...
            _this.otherItemLayout.destroyAllChildren();

            _this.mainItemNode.destroyAllChildren();

            var itemInfo = new (_crd && ItemInfo === void 0 ? (_reportPossibleCrUseOfItemInfo({
              error: Error()
            }), ItemInfo) : ItemInfo)();
            itemInfo.initItemData(_this.firstTable.FRItemIds[0], _this.firstTable.FRItemNum[0]);
            var heroNode = (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
              error: Error()
            }), ItemPoolMgr) : ItemPoolMgr).ins.createRewadItem(itemInfo, _this.mainItemNode);
            var heroTs = heroNode.getComponent(_crd && HeroItem === void 0 ? (_reportPossibleCrUseOfHeroItem({
              error: Error()
            }), HeroItem) : HeroItem);
            ...
            for (var i = 1; i < _this.firstTable.FRItemIds.length; i++) {
              var info = new (_crd && ItemInfo === void 0 ? (_reportPossibleCrUseOfItemInfo({
                error: Error()
              }), ItemInfo) : ItemInfo)();
              info.initItemData(_this.firstTable.FRItemIds[i], _this.firstTable.FRItemNum[i]);
              (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
                error: Error()
              }), ItemPoolMgr) : ItemPoolMgr).ins.createRewadItem(info, _this.otherItemLayout);
            }

            _this.desRichtext.string = (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
              error: Error()
            }), LangMgr) : LangMgr).getLab(_this.firstTable.WordKey);

            if (_this.firstTable.RateShow && _this.firstTable.RateShow != "") {
              _this.discountSpr.node.active = true;

              _this.discountSpr.setTexture(_this.firstTable.RateShow);
            } else {
              _this.discountSpr.node.active = false;
            }

            if (_this.firstTable.AnimationId) {
              if (itemInfo.itemTable.Type == (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                error: Error()
              }), tab) : tab).ItemType.ItemType_Hero) {
                _this.heroNode.active = true;
                _this.propNode.active = false;
                var tempTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                  error: Error()
                }), tab) : tab).getData().AnimationTableById.getValue(_this.firstTable.AnimationId);
                ...
                _this.heroNode.active = false;
                _this.propNode.active = true;

                var _tempTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                  error: Error()
                }), tab) : tab).getData().AnimationTableById.getValue(_this.firstTable.AnimationId);

                if (_tempTab.Type == (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                  error: Error()
                }), tab) : tab).AnimationType.AnimationType_SpriteFrame) {
                  _this.chest_spine.node.active = false;

                  _this.chestSpr.setTexture(_tempTab.Path);
                } else if (_tempTab.Type == (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                  error: Error()
                }), tab) : tab).AnimationType.AnimationType_SkeletonData) {
                  _this.chestSpr.node.active = false;

                  var _spData = yield (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
                    error: Error()
                  }), LoadResAsync) : LoadResAsync)(_tempTab.Path, sp.SkeletonData);

                  _this.chest_spine.skeletonData = _spData;

                  _this.chest_spine.setAnimation(0, _tempTab.AnimationName, true);
                }
              }
            } else {
              _this.hero_spine.node.active = false;
            }

            var rechargeTable = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).getData().RechargeTableById.getValue(_this.firstTable.RechargeId);
            var allRechargeTable = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).getData().RechargeTableById.getValue(1004);
            _this.priceLabel.string = (_crd && ChannelMgr === void 0 ? (_reportPossibleCrUseOfChannelMgr({
              error: Error()
            }), ChannelMgr) : ChannelMgr).getSdkRechargeShowPrice(rechargeTable);
            _this.allPriceLabel.string = (_crd && ChannelMgr === void 0 ? (_reportPossibleCrUseOfChannelMgr({
              error: Error()
