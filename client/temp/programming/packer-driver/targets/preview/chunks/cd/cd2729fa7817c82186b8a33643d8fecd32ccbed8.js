    ...
    execute: function () {
      ...
      _export("HandbookPop", HandbookPop = (_dec = ccclass('HandbookPop'), _dec2 = property(Prefab), _dec3 = property(Node), _dec(_class = (_class2 = class HandbookPop extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        onShow() {
          ...
          var newList = this.sortList(showTabs);
          console.log(newList);

          for (var k = 0; k < newList.length; k++) {
            var _openTab = newList[k];
            var item = instantiate(this.pfb_item);
            item.parent = this.node_content;
            var itemTs = item.getComponent(_crd && HandbookItem === void 0 ? (_reportPossibleCrUseOfHandbookItem({
              error: Error()
            }), HandbookItem) : HandbookItem);
            itemTs.initData(_openTab);
          }
        }
