    ...
    execute: function () {
      ...
      _export("RareBookDetailView", RareBookDetailView = (_dec = ccclass('RareBookDetailView'), _dec2 = property(_crd && RareBookItem === void 0 ? (_reportPossibleCrUseOfRareBookItem({
        ...
        initView() {
          ...
          this.initBaseView();
          var v = (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
            error: Error()
        ...
        initDetailView() {
          ...
          var attrMap = this.currBookInfo.attrMap;
          var index = 0;
          this.hideArrtItem();
          attrMap.forEach((value, key) => {
            var item = this.creatorItem(index);
            item.initView(key, value);
            index++;
          });
          var advanceId = (_crd && RareBookData === void 0 ? (_reportPossibleCrUseOfRareBookData({
            error: Error()
        ...
        initStudyView() {
          ...
          if (!this.currBookInfo.isLevelLimit()) {
            ...
            var nextLevel = this.currBookInfo.level + 1;
            ...
            if (nextLavelTable) {
              var attrMap = this.currBookInfo.attrMap;
              var nextAttrMap = this.currBookInfo.getAttrMapByLevelRatio(nextLavelTable.Ratio);
              var index = 0;
              this.hideArrtItem();
              attrMap.forEach((value, key) => {
                var item = this.creatorItem(index);
              ...
              for (var key in nextLavelTable.MaterialIdList) {
                var id = nextLavelTable.MaterialIdList[key];
                var num = nextLavelTable.MaterialCountList[key];
              ...
              var _attrMap = this.currBookInfo.attrMap;
              var _index = 0;
              this.hideArrtItem();

              _attrMap.forEach((value, key) => {
                var item = this.creatorItem(_index);
            ...
            var _attrMap2 = this.currBookInfo.attrMap;
            var _index2 = 0;
            this.hideArrtItem();

            _attrMap2.forEach((value, key) => {
              var item = this.creatorItem(_index2);
              item.initView(key, value, -1);
              _index2++;
            }); //等级上限
        ...
        initComprehendView() {
          ...
          if (!this.currBookInfo.isMaxStar()) {
            var nextStar = this.currBookInfo.star + 1;
            var nextStarTable = (_crd && RareBookData === void 0 ? (_reportPossibleCrUseOfRareBookData({
              error: Error()
            ...
            if (nextStarTable) {
              ...
              var attrMap = this.currBookInfo.attrMap;
              var ratio = this.currBookInfo.bookLevelTable ? this.currBookInfo.bookLevelTable.Ratio : 0;
              var nextAttrMap = this.currBookInfo.getAttrMapByLevelRatio(ratio, nextStarTable.AttrType, nextStarTable.AttrValue);
              var index = 0;
              this.hideArrtItem();
              attrMap.forEach((value, key) => {
                var item = this.creatorItem(index);
              ...
              for (var key in this.comprehendItems) {
                this.comprehendItems[key].node.active = false;
              }

              index = 0;
              var totoal = 0;
              var isConsumeOther = this.currBookInfo.itemTable.Quality >= (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                error: Error()
              }), tab) : tab).getData().GetKeyValue_ConfigTable().BookStarUniversalMaterialQuality; // for (let key in nextStarTable.MaterialIdList) {

              var id = this.currBookInfo.bookStarTable.MaterialIdList[index];
              var num = this.currBookInfo.bookStarTable.MaterialCountList[index];
              var info = new (_crd && ItemInfo === void 0 ? (_reportPossibleCrUseOfItemInfo({
                error: Error()
              }), ItemInfo) : ItemInfo)();
              ...
              if (this.currBookInfo.itemTable.Quality >= (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                ...
                _info.initItemData(itemId, currNum2);

                if (!this.comprehendItems[index]) {
                  var _item = (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
                    error: Error()
                  }), ItemPoolMgr) : ItemPoolMgr).ins.createItem(_info, this.stuffLayout2, true);

                  _item.parent = this.stuffLayout2;
                  this.comprehendItems.push(_item.getComponent(_crd && CommonItem === void 0 ? (_reportPossibleCrUseOfCommonItem({
                    error: Error()
                  }), CommonItem) : CommonItem));
                } else {
                ...
                _info.needNum = num - currNum;

                if (_info.needNum <= currNum2) {
                  _info.num = _info.needNum;
                } else {
                  _info.num = currNum2;
                }

                this.comprehendItems[index].initData(_info, true, true);
                this.comprehendItems[index].hideConsumeFenMu();
                totoal += currNum2;
              }
            ...
            var _attrMap3 = this.currBookInfo.attrMap;
            var _index3 = 0;
            this.hideArrtItem();

            _attrMap3.forEach((value, key) => {
              var item = this.creatorItem(_index3);
              item.initView(key, value, -1);
              _index3++;
            });
          }
        ...
        onClickUpStar() {
          if (this.currBookInfo.bookStarTable && !this.currBookInfo.isMaxStar()) {
            var id = this.currBookInfo.bookStarTable.MaterialIdList[0];
            var needNum = this.currBookInfo.bookStarTable.MaterialCountList[0];
            var currNum = Number((_crd && ItemData === void 0 ? (_reportPossibleCrUseOfItemData({
              error: Error()
            }), ItemData) : ItemData).ins.getCount(id));
        ...
        updateRed() {
          ...
          var isCanLevel = this.currBookInfo.isLock && this.currBookInfo.isCanStudy;
          var isCanStar = this.currBookInfo.isLock && this.currBookInfo.isCanUpStar;
