        ...
        initView() {
          ...
          this.attrMap.forEach((value, key) => {
            if (key === (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              ...
              var itemTs = item.getComponent(_crd && AssociationAttrPopItem === void 0 ? (_reportPossibleCrUseOfAssociationAttrPopItem({
                error: Error()
              }), AssociationAttrPopItem) : AssociationAttrPopItem);
              itemTs.initData(key, value);
            } else {
              var _item = instantiate(this.pfb_item);

              _item.parent = this.node_content_special;

              var _itemTs = _item.getComponent(_crd && AssociationAttrPopItem === void 0 ? (_reportPossibleCrUseOfAssociationAttrPopItem({
                error: Error()
              }), AssociationAttrPopItem) : AssociationAttrPopItem);

              _itemTs.initData(key, value);
            }
          });
        }
