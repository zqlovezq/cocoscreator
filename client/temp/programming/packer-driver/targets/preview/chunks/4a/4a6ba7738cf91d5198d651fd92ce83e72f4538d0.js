      ...
      _export("RareBookGroupItem", RareBookGroupItem = (_dec = ccclass('RareBookGroupItem'), _dec2 = property(Node), _dec3 = property(Prefab), _dec4 = property(Prefab), _dec5 = property(Node), _dec6 = property(Label), _dec7 = property(Node), _dec8 = property(sp.Skeleton), _dec(_class = (_class2 = class RareBookGroupItem extends Component {
        ...
        initData(groupTabs) {
          ...
          for (var key in table.BookId) {
            ...
            bookInfos.push(info);
            var item = instantiate(this.rareBookItemPrefab);
            item.parent = this.rarebookLayout;
            var com = item.getComponent(_crd && RareBookItem === void 0 ? (_reportPossibleCrUseOfRareBookItem({
              error: Error()
          ...
          for (var _key in groupTabs) {
            var _info = (_crd && RareBookData === void 0 ? (_reportPossibleCrUseOfRareBookData({
              error: Error()
            }), RareBookData) : RareBookData).ins.getSerieInfoById(groupTabs[_key].Id);

            var num = _info ? _info.count : 0;

            var _item = instantiate(this.titleItemPrefab);

            _item.parent = this.attributeNode;

            var _com = _item.getComponent(_crd && RareBookGroupAttributeITitleItem === void 0 ? (_reportPossibleCrUseOfRareBookGroupAttributeITitleItem({
              error: Error()
            }), RareBookGroupAttributeITitleItem) : RareBookGroupAttributeITitleItem);

            _com.initData(groupTabs[_key], num, total);

            this.attrItems.push(_com);
          }
        ...
        updateView() {
          for (var key in this.rareBookItems) {
            this.rareBookItems[key].updateView();
          }

          var total = this.rareBookItems.length;

          for (var _key2 in this.groupTabs) {
            var info = (_crd && RareBookData === void 0 ? (_reportPossibleCrUseOfRareBookData({
              error: Error()
            }), RareBookData) : RareBookData).ins.getSerieInfoById(this.groupTabs[_key2].Id);
            var num = info ? info.count : 0;
            var item = this.attrItems[_key2];
