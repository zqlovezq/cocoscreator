  ...
  return {
    ...
    execute: function () {
      ...
      _export("BattleMainRecordPop", BattleMainRecordPop = (_dec = ccclass('BattleMainRecordPop'), _dec2 = property(ToggleContainer), _dec3 = property(Node), _dec4 = property(_crd && PlayerHeadItem === void 0 ? (_reportPossibleCrUseOfPlayerHeadItem({
        ...
        onToggle(dd) {
          for (var index = 0; index < this.toggleContainer.toggleItems.length; index++) {
            var v = this.toggleContainer.toggleItems[index];
        ...
        updateRole(role) {
          ...
          for (var index = 0; index < role.heroes.length; index++) {
            var v = role.heroes[index];
            totalDamage += v.damage;
            var hero = new (_crd && HeroFightInfo === void 0 ? (_reportPossibleCrUseOfHeroFightInfo({
              error: Error()
          ...
          for (var i = 0; i < this.fightDamageBarNodes.children.length; i++) {
            var _v = this.fightDamageBarNodes.children[i];
            var damage = role.heroes[i].damage || 0;
            _v.getChildByName("damage_bar").getComponent(ProgressBar).progress = Math.floor(damage * 100 / totalDamage) / 100;
            _v.getChildByName("percent_txt").getComponent(Label).string = js.formatStr("%s%", Math.floor(damage * 100 / totalDamage).toString());
          }
