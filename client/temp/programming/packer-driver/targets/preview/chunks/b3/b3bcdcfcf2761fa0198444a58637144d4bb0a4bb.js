    ...
    execute: function () {
      ...
      _export("ActivityData", ActivityData = (_dec = ccclass('ActivityData'), _dec(_class = (_class2 = class ActivityData {
        ...
        addDailyRewardMsg(msg) {
          ...
          var table = tables.find(a => a.Id == msg.id);
        ...
        isOpenDailyAcivity(type) {
          var msg = this.getDailyRewardMsgByType(type);
        ...
        get dailyRewardItemsMap() {
          if (!this._dailyRewardItemsMap) {
            ...
            for (var key in tabs) {
              var list = this._dailyRewardItemsMap.get(tabs[key].Id);
        ...
        getHeroCollectionRewardSucc(id) {
          if (this.heroCollectionMsg) {
            ...
            var index = this.heroCollectionMsg.activatedList.indexOf(id);
        ...
        initOpenActivityMap(actMsg) {
          ...
          for (var key in actMsg) {
            var info = new (_crd && ActivityOpenInfo === void 0 ? (_reportPossibleCrUseOfActivityOpenInfo({
              error: Error()
        ...
        getAllUpTasks(activityId) {
          for (var i = 0; i < this._heroUpArray.length; i++) {
            var activityInfo = this._heroUpArray[i];

            if (activityInfo.TabId === activityId) {
              var actTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                error: Error()
        ...
        getAllUpCanReceiveTasks(activityId) {
          var arr = [];
          var tasks = this.getAllUpTasks(activityId);
        ...
        refresh7GiftPackBuyState() {
          ...
          for (var i = (_crd && DEVELOPTYPE === void 0 ? (_reportPossibleCrUseOfDEVELOPTYPE({
            error: Error()
            ...
            for (var k = 0; k < (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              ...
              var _giftTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                error: Error()
        ...
        getNewPlayerSignAwards() {
          var msg = ActivityData.ins.getDailyRewardMsgByType((_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            error: Error()
          }), tab) : tab).DailyRewardType.DailyRewardType_NewServer);
          ...
          var tabs = ActivityData.ins.getDailyRewrdItemsById(msg.id);

          for (var i = 0; i < tabs.length; i++) {
            var day = tabs[i].Index;
            var index = msg.activatedList.indexOf(day);
        ...
        initActivityHeroGrow(msg) {
          ...
          for (var key in msg.activityHeroGrowMap) {
            var value = msg.activityHeroGrowMap[key];
        ...
        initActivityRecharge(msg) {
          ...
          for (var key in msg.cumulativeRechargeMap) {
            var value = msg.cumulativeRechargeMap[key];
        ...
        getRechargeData(activityId) {
          for (var i = 0; i < this._rechargeArray.length; i++) {
        ...
        refreshHeroGrowData(activityId, star) {
          var heroGrowData = this.getHeroGrowData(activityId);
