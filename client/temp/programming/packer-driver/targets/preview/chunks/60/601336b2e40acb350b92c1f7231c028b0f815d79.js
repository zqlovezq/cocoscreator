  ...
  return {
    ...
    execute: function () {
      ...
      _export("MailPop", MailPop = (_dec = ccclass('MailPop'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec5 = property(Label), _dec6 = property(Label), _dec7 = property(Label), _dec8 = property(Label), _dec9 = property(Node), _dec10 = property(Node), _dec11 = property(Node), _dec12 = property(Node), _dec13 = property(Prefab), _dec14 = property(ScrollView), _dec(_class = (_class2 = class MailPop extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        initView() {
          var mailInfos = (_crd && MailData === void 0 ? (_reportPossibleCrUseOfMailData({
            error: Error()
          ...
          if (mailInfos.length > 0) {
            ...
            for (var key in this.mailItems) {
              this.mailItems[key].node.active = false;
            }

            for (var _key in mailInfos) {
              var item = this.creatorItem(Number(_key));
              item.initData(mailInfos[_key], this.onTouchTitleItem);
            }
        ...
        updateMailContent() {
          if (this.currSelectItem) {
            var info = this.currSelectItem.info;
            ...
            if (info.Type == (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
              ...
              var strs = info.Content.split("|");
              var key = strs[0];
              var valeu = strs.length > 1 ? strs.slice(1, strs.length) : [];
            ...
            var rews = info.Rewards;

            for (var _key2 in rews) {
              var rewInfo = new (_crd && ItemInfo === void 0 ? (_reportPossibleCrUseOfItemInfo({
                error: Error()
              }), ItemInfo) : ItemInfo)();
              rewInfo.merge(rews[_key2]);
        ...
        on_s2c_ReceiveMailsRewardRsp(msg) {
          ...
          if (msg.ids && msg.ids.length > 0) {
            ...
            var _loop = function _loop() {
              var id = Number(msg.ids[key]);

              var item = _this.mailItems.find(a => Number(a.info.id) == id);
