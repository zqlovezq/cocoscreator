      ...
      _export("ItemPoolMgr", ItemPoolMgr = (_dec = ccclass('ItemPoolMgr'), _dec(_class = (_class2 = class ItemPoolMgr {
        ...
        loadItemPrefab() {
          ...
          return _asyncToGenerator(function* () {
            _this.heroPrefab = yield (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
              error: Error()
            }), LoadResAsync) : LoadResAsync)('prefab/common/HeroItem', Prefab);
            _this.conmmonPrefab = yield (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
              error: Error()
            }), LoadResAsync) : LoadResAsync)('prefab/common/CommonItem', Prefab);
            _this.equipmentPrefab = yield (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
              error: Error()
            }), LoadResAsync) : LoadResAsync)('prefab/common/EquipmentItem', Prefab);
            _this.bookPrefab = yield (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
              error: Error()
            }), LoadResAsync) : LoadResAsync)('prefab/common/WeaponItem', Prefab);

            _this.addRef(_this.heroPrefab);

            _this.addRef(_this.conmmonPrefab);

            _this.addRef(_this.equipmentPrefab);

            _this.addRef(_this.bookPrefab);
          })();
        }
        ...
        createRewadItem(reward, parentNode, isTouch) {
          if (isTouch === void 0) {
            isTouch = true;
          }
          ...
          switch (Type) {
            ...
            case (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              ...
              if (itemTab.Id > 6100 && itemTab.Id < 9000) {
                ...
                var _info = new (_crd && HeroInfo === void 0 ? (_reportPossibleCrUseOfHeroInfo({
                  error: Error()
                }), HeroInfo) : HeroInfo)();

                _info.createDefaultData(reward.itemId);

                node = this.createHeroItem(_info, parentNode);
              } else {
                var _itemInfo = new (_crd && ItemInfo === void 0 ? (_reportPossibleCrUseOfItemInfo({
                  error: Error()
                }), ItemInfo) : ItemInfo)();

                _itemInfo.initItemData(reward.itemId, Number(reward.num));

                node = this.createItem(_itemInfo, parentNode, isTouch);
              }
            ...
            case (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              ...
              var euqipInfo = new (_crd && EquipInfo === void 0 ? (_reportPossibleCrUseOfEquipInfo({
                error: Error()
              }), EquipInfo) : EquipInfo)();
        ...
        createItem(data, parentNode, isTouch, isConsume) {
          if (isTouch === void 0) {
            isTouch = true;
          }
        ...
        createEquipItem(data, parentNode, isTouch) {
          if (isTouch === void 0) {
            isTouch = true;
          }
        ...
        createBookItem(data, parent) {
          ...
          node.parent = parent;
          node.setPosition(0, 0);
          var itemTs = node.getComponent(_crd && WeaponItem === void 0 ? (_reportPossibleCrUseOfWeaponItem({
            error: Error()
