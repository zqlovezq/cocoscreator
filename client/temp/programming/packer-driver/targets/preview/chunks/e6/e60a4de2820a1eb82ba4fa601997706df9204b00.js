        ...
        initView(type) {
          ...
          if (this.timeLab) {
            if (Number(this.msg.closeTime) > 0) {
              var lastTimer = Number(this.msg.closeTime) - (_crd && RoleData === void 0 ? (_reportPossibleCrUseOfRoleData({
                error: Error()
          ...
          for (var i = 0; i < this.signItems.length; i++) {
            var _day = this.tabs[i].Index;
            var index = this.msg.activatedList.indexOf(_day);
            var state = 0;

            if (index >= 0) {
              state = 1;
            } else {
              index = this.msg.rewardList.indexOf(_day);
