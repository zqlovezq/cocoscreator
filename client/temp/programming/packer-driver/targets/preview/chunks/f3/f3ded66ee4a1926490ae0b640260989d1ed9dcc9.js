  ...
  return {
    ...
    execute: function () {
      ...
      _export("ForceUpPop", ForceUpPop = (_dec = ccclass('ForceUpPop'), _dec2 = property(Label), _dec3 = property(Label), _dec4 = property(Animation), _dec(_class = (_class2 = class ForceUpPop extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        increaseBattlePower() {
          var duration1 = 0.6; // 前1秒

          var duration2 = 1.2; // 后0.6秒

          var initialPower = this.openData.power;
          var finalPower = this.openData.power + this.openData.addPower;
          var increaseAmount1 = this.openData.addPower * 0.9; // 提升90%的战斗力

          var increaseAmount2 = this.openData.addPower * 0.1; // 提升10%的战斗力
          // 第一个动作，1秒内提升90%的战斗力

          var action1 = tween().to(duration1, {
            value: initialPower + increaseAmount1
          }, {
            progress: (start, end, current, ratio) => {
              var currentValue = start + ratio * (end - start);
              this.updateBattlePower(currentValue);
              return current;
            }
          }); // // 第二个动作，0.6秒内提升剩余的10%的战斗力

          var action2 = tween().to(duration2, {
            value: finalPower
          }, {
            progress: (start, end, current, ratio) => {
              var currentValue = start + ratio * (end - start);
