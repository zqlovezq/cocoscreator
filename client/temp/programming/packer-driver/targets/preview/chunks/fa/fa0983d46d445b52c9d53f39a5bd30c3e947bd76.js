    ...
    execute: function () {
      ...
      _export("PatrolPop", PatrolPop = (_dec = ccclass('PatrolPop'), _dec2 = property(Label), _dec3 = property(Button), _dec4 = property(Sprite), _dec5 = property(Button), _dec6 = property(Sprite), _dec7 = property(Sprite), _dec8 = property(Label), _dec9 = property(Label), _dec10 = property(Label), _dec11 = property(Label), _dec12 = property(Node), _dec13 = property(Node), _dec14 = property(Label), _dec15 = property(Node), _dec16 = property(Node), _dec(_class = (_class2 = class PatrolPop extends (_crd && ViewPop === void 0 ? (_reportPossibleCrUseOfViewPop({
        ...
        onShow() {
          ...
          var stageClearIds = (_crd && BattleMainDataControl === void 0 ? (_reportPossibleCrUseOfBattleMainDataControl({
            error: Error()
        ...
        onDisable() {
          if (this.commonItems && this.commonItems.length > 0) {
            for (var i = 0; i < this.commonItems.length; i++) {
        ...
        StaticView() {
          // 快速巡逻消耗道具
          var quickItem = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
            error: Error()
          }), tab) : tab).getData().GetKeyValue_ConfigTable().QuickPatrolUseItem;
          var quickItemId = quickItem[0];
          var quickItemNum = quickItem[1];
          ...
          if (this.StageId) {
            var patrolTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).getData().PatrolTableByPveStageId.getValue(this.StageId);
            var itemIds = patrolTab.BaseItemIds.concat(patrolTab.ExtraItemIds[0]);
            var itemNums = patrolTab.BaseItemNum.concat(patrolTab.ExtraItemNum[0]);

            for (var i = 0; i < itemIds.length; i++) {
              var ItemTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                error: Error()
              }), tab) : tab).getData().ItemTableById.getValue(itemIds[i]);
              var exp_node = this.node_revenue.children[i];
              var exp_sp = exp_node.getChildByName("icon").getComponent(Sprite);
              exp_sp.setTexture(ItemTab.Icon);
              var exp_lbl = exp_node.getChildByName("label").getComponent(Label);
              var num = 0;
          ...
          var heros = (_crd && HeroTeamControl === void 0 ? (_reportPossibleCrUseOfHeroTeamControl({
            error: Error()
          }), HeroTeamControl) : HeroTeamControl).ins.getTeam();

          for (var _i = 0; _i < this.node_spine.children.length; _i++) {
            var item = this.node_spine.children[_i];
            var spine = item.getComponent(sp.Skeleton);
            var heroInfo = (_crd && HeroData === void 0 ? (_reportPossibleCrUseOfHeroData({
              error: Error()
            }), HeroData) : HeroData).ins.getById(heros[_i].heroId);
          ...
          if (this.commonItems && this.commonItems.length > 0) {
            for (var _i2 = 0; _i2 < this.commonItems.length; _i2++) {
              (_crd && ItemPoolMgr === void 0 ? (_reportPossibleCrUseOfItemPoolMgr({
                error: Error()
              }), ItemPoolMgr) : ItemPoolMgr).ins.putCommonItem(this.commonItems[_i2]);
            }
          }
        ...
        refreshQuickData() {
          ...
          var curQuickTimes = (_crd && ItemData === void 0 ? (_reportPossibleCrUseOfItemData({
            error: Error()
          ...
          var quickItemId = quickItem[0];
          var quickItemNum = quickItem[1];
          var havaItemCount = (_crd && ItemData === void 0 ? (_reportPossibleCrUseOfItemData({
            error: Error()
        ...
        updatePatrolTimes() {
          ...
          var timeCount = curTime - Number(this.startPatrolData.startPatrolTime);
          var maxTime = this.InitialPatrolMaxTime; // 新手没有收益
          ...
          if (patrolTab) {
            var itemIds = patrolTab.BaseItemIds.concat(patrolTab.ExtraItemIds);
            var itemCounts = patrolTab.BaseItemNum.concat(patrolTab.ExtraItemNum);
            var times = patrolTab.BaseTimes.concat(patrolTab.ExtraTimes); // 每个物品的单独计算的cd

            var timesBeginTimes = this.timesBeginTimes;
            var showNothing = true;

            for (var i = 0; i < itemIds.length; i++) {
              var base_timeCount = curTime - Number(timesBeginTimes[i]) > maxTime ? maxTime : curTime - Number(timesBeginTimes[i]);
              var base_time = times[i];
              var base_itemId = itemIds[i];
              var base_itemCount = itemCounts[i];
              ...
              if (this.node_reward_layout.children[i]) {
                var item = this.node_reward_layout.children[i];
                var itemTs = item.getComponent(_crd && CommonItem === void 0 ? (_reportPossibleCrUseOfCommonItem({
                  error: Error()
                }), CommonItem) : CommonItem);
                var curItemCount = itemTs.getItemCount();
                ...
                if (base_time < base_timeCount) {
                  showNothing = false;
                  var base_info = new (_crd && ItemInfo === void 0 ? (_reportPossibleCrUseOfItemInfo({
                    error: Error()
                  }), ItemInfo) : ItemInfo)();
