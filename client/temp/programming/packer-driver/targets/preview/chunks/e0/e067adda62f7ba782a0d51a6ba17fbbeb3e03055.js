    ...
    execute: function () {
      ...
      _export("ItemData", ItemData = class ItemData {
        ...
        adds(list) {
          ...
          var updateItemIds = [];
          var isHaveStamina = false;
          var isHaveChip = false;
          var isUpdateBookRed = false;
          var isBox = false;

          for (var index = 0; index < list.length; index++) {
            var v = list[index];
            var iteminfo = this.getByItemId(v.itemId);

            if (v.itemId == (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
              error: Error()
            }), tab) : tab).CurrencyType.CurrencyType_DailyActivity) {
              var count = iteminfo ? iteminfo.num : 0;
        ...
        isItemsEnough(items) {
          for (var key in items) {
            var currItem = this.getByItemId(items[key].itemId);
        ...
        isItemsEnoughByList(itemIds, counts) {
          for (var key in itemIds) {
            var currNum = this.getCount(itemIds[key]);
