      ...
      _export("EquipInfo", EquipInfo = class EquipInfo extends (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
        ...
        initBaseAttrInfos() {
          ...
          for (var key in this.baseAttr) {
            var info = new (_crd && EquipAttrInfo === void 0 ? (_reportPossibleCrUseOfEquipAttrInfo({
              error: Error()
        ...
        */
        ...
        updateTotalAttr() {
          ...
          for (var key in this.baseAttrInfos) {
            // let table = tab.getData().EquipAttrTableById.getValue(this.baseAttr[key]);
            // let addValue=
            var info = this.baseAttrInfos[key];
            var attrType = info.attrTable.AttrType;
            var addValue = info.getAddValueByLevel(this.enhanceLv);

            if (this.totalAttr.has(attrType)) {
              var value = this.totalAttr.get(attrType);
          ...
          for (var _key in this.extraAttrInfos) {
            var _info = this.extraAttrInfos[_key];
            var _attrType = _info.attrTable.AttrType;

            var _addValue = _info.getAddValueByLevel(this.refineLv);

            if (this.totalAttr.has(_attrType)) {
              var _value = this.totalAttr.get(_attrType);

              this.totalAttr.set(_attrType, _value + _info.attrTable.Base + _addValue);
            } else {
              this.totalAttr.set(_attrType, _info.attrTable.Base + _addValue);
            }
          }
        }
        /**
