System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2", "__unresolved_3", "__unresolved_4", "__unresolved_5", "__unresolved_6", "__unresolved_7", "__unresolved_8", "client_protocol", "__unresolved_9", "__unresolved_10", "__unresolved_11", "__unresolved_12", "__unresolved_13", "__unresolved_14", "__unresolved_15"], function (_export, _context) {
  ...
  function getTimeGuildTXT(timeleft) {
    ...
    let dayStr = (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
      error: Error()
    }), LangMgr) : LangMgr).getLab("Tips_common_day");
    let hourStr = (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
      error: Error()
    ...
    let minuteStr = (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
      error: Error()
    ...
    let sec = hourleft % 60;
    let secondStr = (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
      error: Error()
  ...
  function setGraySpine(sp, isGray) {
    ...
    if (isGray) {
      ...
      let mat = builtinResMgr.get(`ui-sprite-gray-material`);
      ...
      let mat = builtinResMgr.get(`default-spine-material`);
      sp.customMaterial = mat;
    }
  ...
  function sortByVocation(arr) {
    return arr.sort((a, b) => {
      const heroTabA = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
        error: Error()
      }), tab) : tab).getData().HeroTableById.getValue(a);
      const heroTabB = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
        error: Error()
  ...
  function gachaReplace(gachaId, key, cb) {
    const gachaTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
      error: Error()
    ...
    const addCount = count - itemCount; // 转化为代替物
    ...
    const addItemTab = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
      error: Error()
    }), tab) : tab).getData().ItemTableById.getValue(addItemId);
    const addItemNeedCount = addCount * gachaTab.SubItemBaseCount;
    /* 如果拥有的代替物数量够显示弹窗 */

    const havaAddItemCount = (_crd && ItemData === void 0 ? (_reportPossibleCrUseOfItemData({
      error: Error()
  ...
  function dailyBuyShop(itemId, CostCount, getItemId, key, cb, recuitType) {
    ...
    const itemTabName = (_crd && LangMgr === void 0 ? (_reportPossibleCrUseOfLangMgr({
      error: Error()
    ...
    const type = recuitType ? recuitType : (_crd && RecruitType === void 0 ? (_reportPossibleCrUseOfRecruitType({
      error: Error()
  ...
  function handleNumerText(_score) {
    ...
    if (score >= 100000000) {
      let value = String(score / 100000000); // log("valeu==",value);
  ...
  async function createAnimation(animNode, animId, addAnimId, cb) {
    const animaData = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
      error: Error()
    ...
    if (path) {
      switch (animaData.Type) {
        ...
        case (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
          ...
          await (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
            error: Error()
            ...
            if (spine.isValid) {
              ...
              if (addAnimId) {
                const addAnimaData = (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
                  error: Error()
        ...
        case (_crd && tab === void 0 ? (_reportPossibleCrUseOftab({
          ...
          await (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
            ...
            if (spr.isValid && animCom.isValid) {
              ...
              spr.trim = false;
              const anim_clip = AnimationClip.createWithSpriteFrames((_crd && Avatar === void 0 ? (_reportPossibleCrUseOfAvatar({
                error: Error()
  ...
  */
  ...
  function sendChatToGuild(type) {
    const channelId = (_crd && ChatData === void 0 ? (_reportPossibleCrUseOfChatData({
      error: Error()
    ...
    const clientCustomNotice = new (_crd && proto === void 0 ? (_reportPossibleCrUseOfproto({
      error: Error()
  ...
  function formatTimestamp(timestamp) {
    // 创建日期对象，时间戳需要是毫秒
    let date = new Date(timestamp * 1000); // 获取日期中的月、日、小时和分钟

    let month = date.getMonth() + 1; // 月份从0开始，所以要加1

    let day = date.getDate();
    let hours = date.getHours();
    let minutes = date.getMinutes(); // 补0处理，如果是个位数

    let s_month = month < 10 ? '0' + month : month;
    let s_day = day < 10 ? '0' + day : day;
    let s_hours = hours < 10 ? '0' + hours : hours;
    let s_minutes = minutes < 10 ? '0' + minutes : minutes; // 返回格式化后的字符串

    return `${s_month}-${s_day} ${s_hours}:${s_minutes}`;
  }

  function ButtonLock(lockTime = 0.3, callBackFun) {
    ...
    execute: function () {
      ...
      _export("GameUtil", GameUtil = class GameUtil {
        ...
        */
        ...
        static convertNumber(num, isConsume = false) {
          if (num < 10000) {
            return num + "";
