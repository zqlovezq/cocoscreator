System.register(["cc"], function (_export, _context) {
  ...
  function rectPolygon(a, b) {
    ...
    const r3 = tempR4.set(a.xMax, a.y); // intersection check
  ...
  function rectCircle(rect, cp, cr) {
    ...
    const ry = rect.y;
    const rw = rect.width;
    const rh = rect.height; // temporary variables to set edges for testing

    let testX = cx;
    let testY = cy; // which edge is closest?
    ...
    const distance = Math.sqrt(distX * distX + distY * distY); // if the distance is less than the radius, collision!
  ...
  function pointInPolygon(point, polygon) {
    let inside = false;
    const x = point.x;
    const y = point.y; // use some raycasting to test hits
  ...
  return {
    ...
    execute: function () {
      ...
      _export("obbIntersect", obbIntersect = function (centerA, halfA, rotA, centerB, halfB, rotB) {
        let ae0 = halfA.x,
            ae1 = halfA.y,
            ae2 = halfA.z,
            au00 = rotA.m00,
