  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", InfiniteList = (_dec = property({
        ...
        Reload(keepPos = false, allclean = false) {
        ...
        */
        ...
        ScrollToCell(idx, timeInSecond = 1, attenuated = true) {
        ...
        */
        ...
        scrollToOffset(pos, timeInSecond = 1, attenuated = true) {
          this._scrollView.scrollToOffset(pos, timeInSecond, attenuated);
        }
        ...
        _clear(keepPos = false, allclean = false) {
        ...
        *_generatorCell(range) {
        ...
        _execute(generator, duration) {
          return new Promise(resolve => {
            let gen = generator; //创建执行函数

            let func = () => {
              //执行前，先记录开始时间戳
              let startTime = new Date().getTime(); //然后一直从Generator中获取已经拆分好的代码段出来执行
        ...
        async _load() {
          ...
          for (let i = 0; i < dataLen; i++) {
            let s = this._delegate.getCellSize(i); //let s = this.bDynamicSize ? this._preCreateCell(i) : this._delegate.getCellSize(i);
          ...
          this._activeCellIndexRange = range; //分帧加载创建cell
        ...
        async _refreshActiveCells() {
          // update current active cells with new scroll position
          const range = this._getActiveCellIndexRange(); // check if any cell need update


          if (range.equals(this._activeCellIndexRange)) return; // recycle all out of range cell
          ...
          await this._execute(this._generatorActiveCell(range), 1); // update current active cell range
        ...
        *_generatorActiveCell(range) {
        ...
        _recycleCell(cellIndex, allclean = false) {
        ...
        _addCellView(dataIndex) {
          const dataLen = this._delegate.getCellNumber(); // if(dataLen <= 1){
          // 	return;
          // }
        ...
        setAnchorPoint(node, anchorX, anchorY) {
          ...
          let dy = (anchorY - node.getComponent(UITransform).anchorY) * node.getComponent(UITransform).height; // node.getPosition().x += dx;
