  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", Waiting = ccclass(_class = (_class2 = class Waiting extends Component {
         ...
         */
        static async Show(tag, disconectDuration = 15, delay = 1) {
          ...
          let res = (_crd && ResMgr === void 0 ? (_reportPossibleCrUseOfResMgr({
            error: Error()
          }), ResMgr) : ResMgr).get(PREFAB_URL, Prefab);

          if (!res) {
            res = await (_crd && LoadResAsync === void 0 ? (_reportPossibleCrUseOfLoadResAsync({
              error: Error()
        ...
        static WaitPtl(ptl, disconectDuration = 15, delay = 1) {
